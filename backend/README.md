...
## 📱 Connecting Android Device to Backend

### Method A: Over Local Wi-Fi (Recommended)
1. On your PC, check your local IP address:
   ```powershell
   ipconfig
   ```
   Find the **`Wireless LAN adapter Wi-Fi`** IPv4 Address (e.g. `192.168.1.12`).
2. Make sure your server is running with `--host 0.0.0.0`:
   ```powershell
   python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload
   ```
3. In the Android app, go to **⚙️ Paramètres** ➡️ In **URL du serveur**, enter `http://192.168.1.12:8000` (or tap **📶 Mode Wi-Fi**) ➡️ Tap **Enregistrer** ➡️ Tap **🔄 Tester**.

### Method B: Over USB Cable (ADB Reverse)
1. Run on PC:
   ```powershell
   adb reverse tcp:8000 tcp:8000
   ```
2. In Android app **⚙️ Paramètres**, tap **🔌 Mode USB** (`http://127.0.0.1:8000`) ➡️ Tap **Enregistrer** ➡️ Tap **🔄 Tester**.
